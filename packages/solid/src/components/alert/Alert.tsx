...
export function Alert(_props: AlertProps) {
	const [ props, divProps ] = splitProps(
		mergeProps({ variant: 'default' }, _props),
		[ 'variant', 'icon', 'title', 'description', 'children', 'className' ]
	);
	const baseClass = [ "alert", props.variant ].filter(Boolean).join("-");
	return (
		<div {...divProps} role="alert" data-scope="alert" class={cx(baseClass, props.className)}>
			{props.icon !== undefined ? props.icon : icons[ props.variant ]}
			{props.title && <h2 data-scope="alert" data-part="title">{props.title}</h2>}
			{props.description && <section data-scope="alert" data-part="description">{props.description}</section>}
			{props.children && <section data-scope="alert" data-part="content">{props.children}</section>}
		</div>
	);
};
